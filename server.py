...
def stream():
	...
	try:
		url = fix_url(url, request.environ['REMOTE_ADDR'])

		if 'subtitles' in request.query:
			subtitles_url = request.query['subtitles']
			subtitles_url = fix_url(subtitles_url, request.environ['REMOTE_ADDR'])
			logger.debug('Subtitles link is ' + subtitles_url)
			urllib.urlretrieve(subtitles_url, "subtitle.srt")
